...
xlabel('Time \tau [s]'), ylabel('Amplitude'), title('g hat(\tau)');
grid;
...
for index_t=1:length(t)
 ...
 xlabel('Time \tau [s]');
 ylabel('Amplitude');
 title('Signals f(\tau), g hat(\tau+t) and their product');
 hold on
 plot(tau,g_hat(tau -t(index_t)),'r');
 prod = f(tau).*g_hat(tau - t(index_t));
 stem(tau, prod,'g');
 hold off
 ...
 cnv(index_t) = integral(@(tau)f(tau).*g_hat(tau - t(index_t)),-10,10);%
 subplot(212);
 plot(t(1:length(cnv)),cnv);
 xlabel('Time t [s]');
 ylabel('Amplitude');
 title('The correlation (f*g hat)','Interpreter','latex');
